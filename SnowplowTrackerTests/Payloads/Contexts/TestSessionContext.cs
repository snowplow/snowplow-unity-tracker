...
namespace SnowplowTrackerTests.Payloads.Contexts {
	...
	public class TestSessionContext {
		...
		public void TestInitMinimal () {
			SessionContext context = new SessionContext ().SetUserId("userid").SetSessionId("sessionid").SetSessionIndex(30).SetPreviousSessionId(null).SetStorageMechanism(StorageMechanism.Sqlite).Build ();
			...
			Assert.AreEqual ("SQLITE", dict [Constants.SESSION_STORAGE]);
			
			Assert.AreEqual ("iglu:com.snowplowanalytics.snowplow/client_session/jsonschema/1-0-1", context.GetSchema());
			Assert.AreEqual ("{\"data\":{\"sessionIndex\":30, \"sessionId\":\"sessionid\", \"previousSessionId\":null, \"userId\":\"userid\", \"storageMechanism\":\"SQLITE\"}, \"schema\":\"iglu:com.snowplowanalytics.snowplow/client_session/jsonschema/1-0-1\"}", context.GetJson().ToString());
		}
		
		[Test()]
		public void TestInitFull () {
			SessionContext context = new SessionContext ().SetUserId("userid").SetSessionId("sessionid").SetSessionIndex(30).SetPreviousSessionId(null).SetStorageMechanism(StorageMechanism.Sqlite).SetFirstEventId("firstid").Build ();
			Assert.NotNull (context);
			...
			Assert.AreEqual ("iglu:com.snowplowanalytics.snowplow/client_session/jsonschema/1-0-1", context.GetSchema());
			Assert.AreEqual ("{\"data\":{\"sessionIndex\":30, \"sessionId\":\"sessionid\", \"previousSessionId\":null, \"firstEventId\":\"firstid\", \"userId\":\"userid\", \"storageMechanism\":\"SQLITE\"}, \"schema\":\"iglu:com.snowplowanalytics.snowplow/client_session/jsonschema/1-0-1\"}", context.GetJson().ToString());
		}
